...
function confirmAction(message) {
  if (typeof window !== "undefined" && typeof window.confirm === "function") {
    try { return window.confirm(message); } catch { /* Continue with the requested local action. */ }
  }
  return true;
}
...
function deploymentRoute(url) {
  ...
  if (path === "/healthz" || path === "/__internal/provision/initialize") return path;
...
function choosePersonalCloudflareNames(accountId, d1Names, workerNames) {
  const occupied = new Set([...d1Names, ...workerNames]);
  ...
  for (let index = 0; index < 100; index += 1) {
    ...
    const names = { worker: `${CLOUDFLARE_RESOURCE_PREFIX}${suffix}`, d1: `${CLOUDFLARE_RESOURCE_PREFIX}${suffix}` };
    if (![names.worker, names.d1].some((name) => occupied.has(name))) return names;
  }
  throw new CloudflareProvisioningError("RESOURCE_NAME_UNAVAILABLE", "Could not find available Cloudflare resource names");
}

async function initializePersonalWorker(serviceUrl, ownerKey, bootstrapSecret, expiresAt, signature, options = {}) {
  // This claim is single-use. Only the preceding read-only health check retries.
  const payload = await deploymentRequest(`${serviceUrl}/__internal/provision/initialize`, {
    method: "POST",
...
async function provisionPersonalCloudflare(accessToken, onStatus = () => {}, options = {}) {
  ...
  const names = choosePersonalCloudflareNames(
    accountId,
    (d1Databases || []).map((value) => String(value.name || "")),
    (workerNames || []).map((value) => String(value.id || value.name || value)),
  );
  let createdD1;
  ...
  try {
    ...
    createdD1 = String(database?.uuid || database?.database_id || database?.id || "");
    ...
    step("migration");
    for (const statement of splitMigrationStatements(EMBEDDED_TARGET_MIGRATION_SQL)) {
      const queries = await cf(`/accounts/${encodeURIComponent(accountId)}/d1/database/${encodeURIComponent(createdD1)}/query`, { method: "POST", body: JSON.stringify({ sql: statement, params: [] }) });
    ...
    const initialized = await initializePersonalWorker(serviceUrl, ownerKey, bootstrapSecret, expiresAt, signature, options);
...
const COPY = {
  ...
  open: "Visit",
  settingsIntro: "Publish Note turns the active Obsidian Markdown note into a shareable website. It uses the active note as the share root, can include linked notes to the depth you choose, uploads referenced local assets, and copies a stable link after publishing.",
  settingsIntroDetails: "Use the settings below to connect your own Cloudflare account and choose the publishing scope. Publish from the command palette, the ribbon icon, or a note's context menu.",
  language: "Language",
  ...
  selfCloudflareDescription: "Publish through a Worker and D1 database in your Cloudflare account. Notes and referenced local assets stay in your account; personal deployment does not create or require R2.",
  officialCloudflareDescription: "Use the official Publish Note Cloudflare service. Authorize once, then publish from Obsidian.",
  ...
  deploymentProvisioning: "Creating your Worker and D1 database...",
  ...
  disconnectConfirm: "Disconnect this Vault from Cloudflare? This removes only the saved Worker address and Publish Token from this Vault. Cloudflare resources and published sites will not be deleted.",
  ...
  deploymentStageErrors: { accounts: "The plugin could not read the authorized Cloudflare accounts.", resources: "The plugin could not inspect Cloudflare resources.", database: "The plugin could not create or identify the D1 database.", subdomain: "The plugin could not configure the Worker address.", worker_upload: "The plugin could not attach the D1 database to the Worker while uploading it.", worker_enable: "The plugin could not enable the Worker address.", migration: "The plugin could not apply the D1 database schema.", ready_check: "The plugin could not confirm that the Worker is ready.", initialize: "The plugin could not create the first Publish Note account.", bootstrap_cleanup: "The plugin could not remove the temporary initialization secret.", authorization: "The plugin could not complete Cloudflare authorization." },
  ...
  connectedNotice: "Publish Note account connected.",
...
const COPY_ZH = {
  ...
  repository: "项目仓库",
  open: "访问",
  settingsIntro: "Publish Note 可以将当前打开的 Obsidian Markdown 笔记转换为可分享的网站。当前笔记会作为分享根页面，可以按你选择的深度携带链接笔记，上传笔记引用的本地资源，并在发布后复制稳定链接。",
  settingsIntroDetails: "请在下面连接你自己的 Cloudflare 账户并选择发布范围。你可以从命令面板、功能区图标或笔记右键菜单发起发布。",
  language: "语言",
  ...
  selfCloudflareDescription: "通过你 Cloudflare 账户中的 Worker 和 D1 发布。笔记及引用的本地资源保存在你的账户中；个人部署不会创建或要求 R2。",
  officialCloudflareDescription: "使用官方 Publish Note Cloudflare 服务。授权一次后即可从 Obsidian 发布。",
  selectedMode: "当前选中",
  ...
  deploymentProvisioning: "正在创建 Worker 和 D1 数据库……",
  ...
  disconnectConfirm: "要取消此 Vault 与 Cloudflare 的连接吗？这只会删除此 Vault 保存的 Worker 地址和 Publish Token，不会删除 Cloudflare 资源或已发布网站。",
  ...
  deploymentStageErrors: { accounts: "插件无法读取已授权的 Cloudflare 账户。", resources: "插件无法检查 Cloudflare 资源。", database: "插件无法创建或识别 D1 数据库。", subdomain: "插件无法配置 Worker 地址。", worker_upload: "插件在上传 Worker 时无法连接 D1 数据库。请确认 D1 数据库已创建且绑定信息有效。", worker_enable: "插件无法启用 Worker 地址。", migration: "插件无法执行 D1 数据库迁移。", ready_check: "插件无法确认 Worker 已就绪。", initialize: "插件无法创建首个 Publish Note 账户。", bootstrap_cleanup: "插件无法删除临时初始化凭证。", authorization: "插件无法完成 Cloudflare 授权。" },
  notDeployed: "尚未部署",
  ...
  connectedNotice: "Publish Note 账户已连接。",
...
class SharePublisherPlugin extends Plugin {
  ...
  async disconnectCloudflare() {
    ...
    if (!confirmAction(this.copy().disconnectConfirm)) return false;
...
class SharePublisherSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    const cloudflareSetting = new Setting(containerEl)
      ...
      .setDesc(`${copy.selfCloudflareDescription} ${copy.deploymentDescription} ${!hasPersonal && !isDesktopEnvironment() ? copy.desktopDeploymentOnly : ""} ${personalSelected ? copy.selectedMode : copy.notSelectedMode}${session?.active ? ` · ${deploymentProgress(session.stage, copy)}` : ""}${session?.error ? ` · ${deploymentFailure(session.error, copy)}` : ""}`)
    ...
    if (hasPersonal) cloudflareSetting.addButton((button) => button
      .setButtonText(copy.disconnectCloudflare)
      .setDisabled(cloudflareBusy)
      .onClick(() => void this.plugin.disconnectCloudflare().then(() => this.display())));
