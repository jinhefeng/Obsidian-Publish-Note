...
- In scope: 官方托管与用户自部署 Worker、官方服务控制面、桌面插件直连 Cloudflare OAuth/API 的个人部署、单页入口、引用页面遍历、本地内容编译、随机不透明 siteId、发布包契约、账户/设备授权/Publish Token、官方 D1/R2 与个人 D1-only 站点存储、网页访问和失败安全切换。
...
| Portable Publish/Auth Core | 账户、恢复码、设备授权、Token、上传会话、配额、提交和 viewer 查询 | T3 | `PublishStorage`, Web Crypto | service API |
...
- Constraint: C-005 protocol v2 对 UTF-8 页面按文本片拼接，对 base64 二进制资源要求每片独立解码；Worker 不接收完整 50MB bundle。
- Constraint: 每个账户最多 50MB 当前内容和 10 个 Note；更新按“其他站点占用 + 新 revision 大小”预检查。
