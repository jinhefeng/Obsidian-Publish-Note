# One-Click Publish — Agent Instructions
...
- Product name: `One-Click Publish`.
- Author: `Jin Hefeng`.
...
- The note context-menu item must be named exactly `One-Click Publish`.
- The command palette and ribbon publish entry should remain consistent with `One-Click Publish`.
- Do not reintroduce a `Publish current note` or `Last published link` block into the settings page.
- Do not add a first-line `One-Click Publish` heading to the settings page. The first content should be the product introduction.
- The settings page should begin with a moderately detailed explanation of what One-Click Publish does: after a one-time Cloudflare setup, it publishes the active Markdown note with one click, can include linked notes, uploads referenced local assets, and copies a stable link after publishing.
...
- Root and `plugin/` README files are user-facing plugin documentation. Keep project development, testing, packaging, and release procedures in `CONTRIBUTING.md` and `CONTRIBUTING.zh-CN.md` instead of making the README read like an engineering project overview.
...
- Use the language-specific copy map for settings labels, descriptions, and notices. Keep the context-menu command exactly `One-Click Publish` as required above.
...
- The local publishing service is an in-memory development service. Do not describe it as production hosting. The official hosted path may use Cloudflare Worker/R2/D1 and uses the official One-Click Publish control plane. Personal deployment runs directly from Obsidian desktop through Cloudflare OAuth/API and uses only Worker/D1 BLOB storage: it must not request, create, or require R2. Deploy Button, Wrangler, and manual `/setup` remain advanced fallbacks.
...
- Every plugin version update must synchronize all of these artifacts from the `plugin/` sources:
  ...
  - `Vault/.obsidian/plugins/one-click-publish/manifest.json` and `Vault/.obsidian/plugins/one-click-publish/main.js` for local smoke testing.
...
Then reload `One-Click Publish` in Obsidian and perform a real plugin smoke test when the task requires runtime verification.
