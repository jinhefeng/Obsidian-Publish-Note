...
const EMBEDDED_TARGET_PLUGIN_VERSION = "0.2.24";
const EMBEDDED_TARGET_ARTIFACT_HASH = "dda94dbb548a9285";
...
async function findHistoricalPersonalD1(cf, accountId, databases, workerNames) {
  ...
  throw new CloudflareProvisioningError("MULTIPLE_HISTORICAL_DATABASES", "More than one historical Publish Note database was found; deployment was stopped to protect existing content", { stage: "resources" });
}
...
const COPY = {
  ...
  selfCloudflareDescription: "Publish through a Worker and D1 database in your Cloudflare account. The deployment prefers the stable Worker name publish-note and reuses a recognized historical Publish Note D1 when reconnecting; only an unrelated name conflict causes a predictable suffix. Notes and referenced local assets stay in your account; personal deployment does not create or require R2.",
  ...
  deploymentStageErrors: { accounts: "The plugin could not read the authorized Cloudflare accounts.", resources: "The plugin could not inspect Cloudflare resources or identify a historical Publish Note database.", database: "The plugin could not create or identify the D1 database.", subdomain: "The plugin could not configure the Worker address.", worker_upload: "The plugin could not attach the D1 database to the Worker while uploading it.", worker_enable: "The plugin could not enable the Worker address.", migration: "The plugin could not apply the D1 database schema.", ready_check: "The plugin could not confirm that the Worker is ready.", initialize: "The plugin could not create or reconnect the Publish Note account.", bootstrap_cleanup: "The plugin could not remove the temporary initialization secret.", authorization: "The plugin could not complete Cloudflare authorization." },
...
const COPY_ZH = {
  ...
  selfCloudflareDescription: "通过你 Cloudflare 账户中的 Worker 和 D1 发布。部署会优先使用固定的 Worker 名称 publish-note，重新连接时会复用已识别的历史 Publish Note D1；只有无关资源同名时才使用可预测的后缀。笔记及引用的本地资源保存在你的账户中；个人部署不会创建或要求 R2。",
  ...
  deploymentStageErrors: { accounts: "插件无法读取已授权的 Cloudflare 账户。", resources: "插件无法检查 Cloudflare 资源或识别历史 Publish Note 数据库。", database: "插件无法创建或识别 D1 数据库。", subdomain: "插件无法配置 Worker 地址。", worker_upload: "插件在上传 Worker 时无法连接 D1 数据库。请确认 D1 数据库已创建且绑定信息有效。", worker_enable: "插件无法启用 Worker 地址。", migration: "插件无法执行 D1 数据库迁移。", ready_check: "插件无法确认 Worker 已就绪。", initialize: "插件无法创建或恢复 Publish Note 账户。", bootstrap_cleanup: "插件无法删除临时初始化凭证。", authorization: "插件无法完成 Cloudflare 授权。" },
  notDeployed: "尚未部署",
